...
public class LivraisonControleur implements Initializable {
	...
	/**
	 * La methode ModifierLivraison permet de gerer les differents cas de
	 * modifications de livraisons depuis l'IHM.
	 */
	public void ModifierLivraison() {
		...
		if (!comboAHeur.getSelectionModel().isEmpty() && !comboAHeur.getSelectionModel().getSelectedItem().equals("--")
				&& !comboAMinute.getSelectionModel().isEmpty()
				&& !comboAMinute.getSelectionModel().getSelectedItem().equals("--")
				&& !comboDeHeur.getSelectionModel().isEmpty()
				&& !comboDeHeur.getSelectionModel().getSelectedItem().equals("--")
				&& !comboDeMinute.getSelectionModel().isEmpty()
				&& !comboDeMinute.getSelectionModel().getSelectedItem().equals("--")) {
		...
		if (aController.getTournee() == null) {
			...
			aController.getDemandeLiv().getLivraisons().set(idx, livraison);
			listeDeCdes
					.ajoute(new CdeModificationDuree(plan, aController.getTournee(), livraison, dureeA, DPH_A, FPH_A));
			aController.mettreAJour();
	...
	/**
	 * La methode SupprimerLivraison permet de gerer la suppresion d une
	 * livraison depuis l'IHM.
	 */
	public void SupprimerLivraison() {
		try {
			...
			if (aController.getTournee() != null) {
				Pair<Integer, Tournee> paire = aController.getTournee().SupprimerLivraison(plan, intersection,
						livraison);
	...
	/**
	 * La methode AjouterLivraison permet de gerer la suppresion d une livraison
	 * depuis l'IHM.
	 */
	public void AjouterLivraison() {

		try {
			if (!comboAHeur.getSelectionModel().isEmpty()
					&& !comboAHeur.getSelectionModel().getSelectedItem().equals("--")
					&& !comboAMinute.getSelectionModel().isEmpty()
					&& !comboAMinute.getSelectionModel().getSelectedItem().equals("--")
					&& !comboDeHeur.getSelectionModel().isEmpty()
					&& !comboDeHeur.getSelectionModel().getSelectedItem().equals("--")
