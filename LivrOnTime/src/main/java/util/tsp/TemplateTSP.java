...
public abstract class TemplateTSP implements TSP {
	 ...
	 */
	protected abstract Iterator<Integer> iterator(Integer sommetCrt, ArrayList<Integer> nonVus, long[][] cout,
			long[] duree);
	 ...
	 */
	void branchAndBound(int sommetCourant, ArrayList<Integer> nonVus, ArrayList<Integer> vus, long coutVus,
			long[][] cout, long[] duree, long tpsDebut, long tpsLimite, long[][] temps) {
		...
		if (nonVus.size() == 0) { // tous les sommets ont ete visites
			...
			while (it.hasNext()) {
				...
				// Code simple permettant de savoir si la plage horaire est
				// respect�e
				if ((temps[prochainSommet][1]) > coutVus + cout[sommetCourant][prochainSommet]) {
					if ((temps[prochainSommet][0]) > coutVus + cout[sommetCourant][prochainSommet]) {
						...
						branchAndBound(prochainSommet, nonVus, vus, temps[prochainSommet][0], cout, duree, tpsDebut,
								tpsLimite, temps);
					} else {
						branchAndBound(prochainSommet, nonVus, vus,
								coutVus + cout[sommetCourant][prochainSommet] + duree[prochainSommet], cout, duree,
								tpsDebut, tpsLimite, temps);
					}
				}
